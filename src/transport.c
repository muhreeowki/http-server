...
int listen_and_accept(struct transport *t);
...
int main(int argc, char *argv[]) {
  ...
  if ((trans = new_transport(argv[1], "3000", &hints, &nop_handler)) == NULL) {
    exit(EXIT_FAILURE);
  }
  if (listen_and_accept(trans) == -1) {
    exit(EXIT_FAILURE);
  }
  close(trans->sock_fd);
  return EXIT_SUCCESS;
...
struct transport *new_transport(char *host, char *service,
  ...
  if ((gai_status = getaddrinfo(host, service, hints, &servinfo) != 0)) {
